...
//            ThrowUtils.throwIf(!exists,ErrorCode.NOT_FOUND_ERROR,"图片不存在");
    ...
    public void doPictureReview(PictureReviewRequest pictureReviewRequest, User user) {
        ...
        if (reviewStatusEnum == null || reviewStatusEnum.equals(PictureReviewStatusEnum.REVIEWING) || id == null) {
        ...
        if (oldPicture.getReviewStatus().equals(reviewStatus)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "不可重复申请");
        }
        //数据库操作
        Picture picture = new Picture();
        BeanUtils.copyProperties(pictureReviewRequest, picture);
        picture.setId(user.getId());
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        Document document;
        try {
            document = Jsoup.connect(fetchUrl).get();//链接地址后得到他的文档,document可理解为包含所有的html
        } catch (IOException e) {
            log.error("获取页面失败：{}", e);
        ...
        for (Element imgElement : imgElementList) {
            ...
            if (StrUtil.isBlank(fileUrl)) {
                log.info("此链接为空：{}", fileUrl);
                continue;
            }
            //处理图片的地址，防止转义或对象存储冲突问题
            //codeFather.cn?zzx=handsomeBoy  只保存codeFather.cn   问好后面是查询条件
